...
#### You have to add bellow code on app/gradle file
...
#### Manifest setting for access location

You have to add below permission in your `AndroidManifest.xml`
...
Add settings for webview to access storage in `onCreate()` method in your web activity.
```java
...
Set the permission of location in `onRequestPermissionsResult()` in the web view activity like -
```java
switch (requestCode){
...
To check the location setting are enabled or not in web view activity with below code.
```java
...
#### 3.Open link in browser from webview
...
#### 4.Open webview menu from native
...
#### Set Common Alert, Its using on all places like force location permission, camera & gallery permission error etc..
```java
